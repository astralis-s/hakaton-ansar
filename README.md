...
## Telegram-бот поддержки (опционально)

Заказчики пишут Telegram-боту — их сообщения попадают в **тот же стафф-чат**, что и
обычные обращения (`/api/app/chats`), а ответы менеджера с сайта доставляются обратно
в Telegram. При первом запуске бот собирает **ФИО и телефон**: по ним заводится клиент
в CRM, поэтому в списке чатов менеджер видит реальное ФИО, а не никнейм Telegram.

Это отдельный модуль `internal/modules/telegrambot` (long polling, без вебхуков),
который тонким адаптером переиспользует чат `portal` и создание клиента в `crm` —
доменную логику не дублирует. Бот **опционален**: без `TELEGRAM_BOT_TOKEN` приложение
работает как прежде.
