...
### Фаза 7 — Демо-данные и финал ✅
- **Сидер** `internal/seed` (грозненский колорит, идемпотентный — пропускает, если организация уже есть): организация «Грозный Мебель», владелец + менеджер (bcrypt), **известный демо `X-API-Key`** (`amana_demo_marketplace_key_2026`), 3 клиента, 5 товаров (разные халяль-статусы), **3 договора в разных состояниях** (частично оплаченный; просроченный с начисленной садакой; свежий), 3 напоминания (часть со сдвигом мимо намаза/джума). Строится через те же app/domain-конструкторы → данные консистентны.
- **Запуск сида**: флаг `SEED_ON_BOOT` (в `docker-compose.yml` = `true`); демо-доступы логируются на старте.
- **README**: команда запуска (`docker compose up`), демо-доступы, демо-сценарий на 3 минуты, обзор двух поверхностей API, доменные правила, архитектура, make-таргеты, тесты.
- **Финальная проверка всего стека** (реальный PG 16, путь `docker compose up`): boot → миграции (до v6) → **сид** (орг/2 пользователя/демо-ключ/3 клиента/5 товаров/3 договора/3 напоминания) → owner+manager логин; `/api/app/contracts`=3 active; реестр садаки total `500.00`; `/api/app/schedule/reminders`=3, **2 сдвинуты** (причины «намаз Аср», «джума»); каталог=5, клиенты=3; публичный `GET /api/v1/.../payments` по демо-ключу→200; харам через публичный create→409; Swagger отдаёт `/contracts`, UI=200; **повторный старт → «seed: skipped»** (идемпотентность). Ошибок/паник нет.

---

## Итог реализации бэкенда

Все 7 фаз закрыты: модульный монолит на чистой архитектуре (5 bounded contexts +
публичный API), вся доменная математика (мурабаха, намаз) покрыта обязательными
table-driven тестами и проверена сквозными прогонами против реального PostgreSQL 16.
`go build ./...`, `go vet ./...`, `go test ./...` — зелёные. Запуск — одной командой
`docker compose up` (миграции + демо-сид на старте). Примечание: сборка docker-образа
и `docker compose up` как таковые в CI-песочнице без docker-демона не запускались —
проверки выполнялись тем же кодовым путём против локально поднятого PostgreSQL 16;
`Dockerfile`/`docker-compose.yml` написаны и валидны.

---

## Фронтенд (SPA) и финальная интеграция

Реализован самостоятельный фронтенд `web/`, **подключённый к реальному API** и
**встроенный в Go-бинарь** (`//go:embed`) — проект отдаётся одной командой целиком.
Демонстрационный исходник дизайна (проприетарный формат артефакта на `support.js`)
**не переносим**, поэтому собран чистый SPA на его дизайн-системе.

- **Технологии без сборки**: React 18 + `htm` (вендорятся локально в `web/assets/vendor`,
  без рантайм-CDN), `<script>`-модули, hash-роутинг. Дизайн-система — токены палитры
  (мурабаха-зелёный, свет/тьма), IBM Plex Sans, keyframes, статус-цвета.
- **Лендинг переписан** (приоритет задачи): анимированный mesh-фон, свечение под курсором,
  параллакс, scroll-reveal (IntersectionObserver), анимированный градиент, hero с плавающей
  карточкой договора, секции «возможности» и «как это работает», **анимированное сравнение
  «без рибы vs кредит»** (SVG), CTA, футер. Демо-данные с «5+ намазов» удалены, содержание
  переписано под продукт. Модалка **входа/регистрации** подключена к `POST /api/app/auth/login`
  и `POST /api/app/setup` (первичная настройка организации).
- **Приложение подключено к бэкенду** (без mock): дашборд (KPI из реальных списков),
  клиенты/каталог (CRUD), **мастер договора** с live-предпросмотром через
  `POST /api/app/contracts/preview` (график + сравнение), **карточка договора** (приём платежа
  любой суммой, статусы долей, досрочное погашение, отмена и начисление садаки — owner-only,
  баннер просрочки), **календарь** с предпросмотром сдвига мимо намаза
  (`POST /api/app/schedule/preview`), **реестр садаки**, **API-ключи** (создать/показать один раз/
  отозвать, ссылка на Swagger), настройки. Деньги — строки-decimal, форматирование `Intl`.
- **Сервинг из Go**: `web.Handler()` отдаёт `index.html` и `/assets/*` из эмбеда; SPA-fallback
  для deep-link, при этом `/api/*` и `/swagger/*` сохраняют свои ответы. `.dockerignore`
  поправлен (web/ участвует в сборке образа).
- **Проверки**: `node --check` всех модулей; node-смоук рендера 13 компонентов (лендинг + 10
  экранов + примитивы) — без ошибок; рантайм против реального PG 16: `/` и все ассеты `200`,
  deep-link `/clients`→SPA, `/api/nope`→JSON 404, `/swagger/`→200, логин демо-владельцем,
  3 сид-договора видны. Браузерная визуальная проверка в песочнице невозможна (нет GUI) —
  отмечено честно; код провалидирован статически и смоук-рендером.

### Переработка дашборда

- **Новый агрегирующий эндпоинт** `GET /api/app/dashboard` (financing): тонкий хендлер
  (парс → app-сервис → маппинг), вся арифметика — в чистом доменном `BuildDashboard`
  (деньги через `Money`/`decimal`, на границе — строки-decimal). Отдаёт: портфель (остаток +
  число активных), неделя (ожидается/собрано/collection rate + ближайшие платежи), просрочка
  (имя клиента · остаток · дни). Имена клиентов — через расширенный порт `ClientReader.Names`
  (один запрос), договоры — `ContractRepository.ListFullByOrg`. Доменный table-тест на
  агрегацию (портфель, дни просрочки, expected/collected, rate, сортировка) — зелёный.
- **Экран дашборда переписан** по приоритету «что горит → что делать → масштаб»: блок
  **просрочки** (имена/суммы/дни, клик → карточка договора) самый заметный; **к получению
  на неделе** + collection rate с прогрессом и списком ближайших платежей; **повестка на
  сегодня** с пометкой «перенесено из-за намаза»; спокойный **портфель** (остаток + счётчик
  активных подписью, не отдельной плиткой); быстрые действия («Новый договор/клиент»,
  **«Принять платёж»** — выбор договора → модалка приёма). Убраны «Собрано садаки»,
  «Последние договоры» и отдельная KPI «Активные договоры».
- **Состояния**: Skeleton (загрузка), EmptyState для новой организации, per-block ошибка с
  «Повторить» (падение одного источника не роняет экран). Деньги только отображаются
  (`fmt.money`), на фронте не считаются. Адаптация под наш no-build SPA: «query-хуки» →
  `useAsync`+`AM.api`; «Vitest/RTL» → Go-доменный тест агрегации + live-проверка эндпоинта +
  node-смоук (Vitest в no-build SPA не настроен).
- **Проверка рантайма** (реальный PG 16 + сид): `/api/app/dashboard` → портфель 260750.00 /
  3 активных; просрочка «Хеда Юсупова · 144000.00 · 243 дн.»; неделя expected/collected;
  имена клиентов резолвятся. Ошибок нет.
