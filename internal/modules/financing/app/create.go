...
// contract; the Draft state is internal). A Haram product is refused.
...
func NewCreateContract(contracts domain.ContractRepository, products domain.ProductReader, clients domain.ClientReader, tx domain.TxManager) *CreateContract {
	return &CreateContract{contracts: contracts, products: products, clients: clients, tx: tx}
}
